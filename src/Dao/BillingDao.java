...
public class BillingDao {
	...
	public static ArrayList<LogCall> getListLogCall_CT(String idcongty) {
		...
		try {
			...
			while (rs.next()) {
				...
				listLogCall_CT.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));
			}
			statement.close();
			connection.close();
		} catch (Exception e) {
	...
	public static ArrayList<LogCall> getListLogCall_PB(String idphongban) {
		...
		try {
			...
			while (rs.next()) {
				...
				listLogCall_PB.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));
			}
			statement.close();
			connection.close();
		} catch (Exception e) {
	...
	public static ArrayList<LogCall> getListLogCall_EX(String idextension) {
		...
		try {
			...
			while (rs.next()) {
				...
				listLogCall_EX.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));
			}
			statement.close();
			connection.close();
		} catch (Exception e) {
	...
	public static ArrayList<LogCall> getListLogCall_DV(String dichvu, String idcongty) {
		...
		try {
			...
			while (rs.next()) {
				...
				listLogCall_DV.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));;
			}
			statement.close();
			connection.close();
		} catch (Exception e) {
		...
		public static ArrayList<LogCall> getListLogCall_DVPB(String dichvu, String idphongban) {
			...
			try {
				...
				while (rs.next()) {
					...
					listLogCall_DVPB.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));
				}
				statement.close();
				connection.close();
			} catch (Exception e) {
				...
				public static ArrayList<LogCall> getListLogCall_DVEX(String dichvu, String extension) {
					...
					try {
						...
						while (rs.next()) {
							...
							listLogCall_DVEX.add(new LogCall(congtyid, tencongty, extensionID, thuebaonhan, phongbanid, ten_phongban, thoigian_goi, thang, loai_cuocgoi, gia_tien, thoigianbatdau, thoigianketthuc));
						}
						statement.close();
						connection.close();
					} catch (Exception e) {
	...
	//TINH TONG TIEN CUA tung phong ban theo dich vu
		public static double tinhtongtienpb(String loaicuocgoi, String phongbanid) {
			...
			try {
				Connection connection = DatabaseSQLConnection.getConnection();
				Statement statement = connection.createStatement();
				String sql = "select  sum(gia_tien) tongtien from log_call lc join extension ex on lc.extensionID = ex.extensionID join phongban pb on ex.phongbanID = pb.phongbanID where loai_cuocgoi = '"+loaicuocgoi+"'and pb.phongbanID = '"+phongbanid+"' group by loai_cuocgoi;";
				ResultSet rs = statement.executeQuery(sql);
				while (rs.next()) {
					tongtien = rs.getDouble("tongtien");
				}
		...
		public static double tinhtongtienex(String loaicuocgoi, String extension) {
			double tongtien = 0;
					...
					try {
						Connection connection = DatabaseSQLConnection.getConnection();
						Statement statement = connection.createStatement();
						String sql = "select  sum(gia_tien) tongtien from log_call lc join extension ex on lc.extensionID = ex.extensionID join phongban pb on ex.phongbanID = pb.phongbanID where loai_cuocgoi = '"+loaicuocgoi+"'and ex.extensionID = '"+extension+"' group by loai_cuocgoi;";
						ResultSet rs = statement.executeQuery(sql);
						while (rs.next()) {
							tongtien = rs.getDouble("tongtien");
						}
				...
				public static double tinhtongtienall(String loaicuocgoi) {
					double tongtien = 0;
							...
							try {
								Connection connection = DatabaseSQLConnection.getConnection();
								Statement statement = connection.createStatement();
								String sql = "select  sum(gia_tien) tongtien from log_call lc join extension ex on lc.extensionID = ex.extensionID join phongban pb on ex.phongbanID = pb.phongbanID where loai_cuocgoi = '"+loaicuocgoi+"'group by loai_cuocgoi;";
								ResultSet rs = statement.executeQuery(sql);
								while (rs.next()) {
									tongtien = rs.getDouble("tongtien");
								}
	...
	// TINH TONG TIEN MA TONG CONG TY PHAI TRA CHO TUNG NHA MANG
//		public static double totalnm(String nhamang,int thang){
//			ArrayList<CongTy> listCongTy = new ArrayList<CongTy>();
//			listCongTy = CongTyDao.getListCongTy();
//			double total =0;
//			for (int i = 0; i < listCongTy.size(); i++) {
//				if(listCongTy.get(i).get)
//				total += BillingDao.totalct(listCongTy.get(i).getCongTyID(), thang);
//			}
//			return total;
//		}
	// TINH TONG TIEN cuoc goc cua dau so theo nha mang
		public static double tongcuocdausonm(String dauso,String nhamang, int thang){
			...
			try {
				Connection connection = DatabaseSQLConnection.getConnection();
				Statement statement = connection.createStatement();
				String sql = "select  sum(gia_tien) tongtien from log_call lc join extension ex on lc.extensionID = ex.extensionID where nhamang_id = '"+nhamang+"' and dauso_sudung = '"+dauso+"'  group by dauso_sudung;";
				ResultSet rs = statement.executeQuery(sql);
				while (rs.next()) {
					tongtien = rs.getDouble("tongtien");
				}
